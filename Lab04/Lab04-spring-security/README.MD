...
### Prestamos, amortizacion y abonos

- `POST /api/prestamos`
  - crea un prestamo propio en estado `PENDIENTE`.
  - `ADMIN` puede indicar `usuario_id` para asignarlo a otro usuario.
- `GET /api/prestamos`
  - devuelve una respuesta paginada; `ADMIN` ve todos y `USER` solo los propios.
- `GET /api/prestamos/{id}`
  - obtiene un prestamo respetando el aislamiento por usuario.
- `PUT /api/prestamos/{id}/estado`
  - al cambiar a `APROBADO`, genera una tabla de amortizacion de cuota fija.
- `GET /api/prestamos/{id}/plan-pagos`
  - devuelve las cuotas de amortizacion paginadas.
- `POST /api/planes-pago/{planPagoId}/abonos`
  - registra abonos parciales o totales dentro de una transaccion.
- `GET /api/planes-pago/{planPagoId}/abonos`
  - devuelve los abonos paginados.

La coleccion importable para pruebas se encuentra en `insomnia-collection.json`.
